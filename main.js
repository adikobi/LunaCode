 ...
 */
function runCode() {
    // Reset only the character's position before running the code.
...
/**
 * Resets the level to its initial state (character and workspace).
 */
function resetLevel() {
    resetCharacterPosition();
    Blockly.Xml.clearWorkspace(workspace);
    // The toolbox is technically a workspace too, so we need to add the blocks back.
    // In a more complex app, we would manage toolbox state better.
    const toolbox = {
        'kind': 'flyoutToolbox',
        'contents': [
            {
...
/**
 * Resets only the character's position to the start.
 */
